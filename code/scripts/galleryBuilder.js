...
onTextChange:function(text){
	...
	generateranges:function(template,pointer,ranges){
	...
	for(i=0;i<range.length;i++){
		...
		currentval= i or mon[i] or char[i]
		
		curenturl=curenturl.replace("[]",currentval)
		
		if(curenturl.contains("[]"))
			results.push (recursive(app))
		else
			results.push(curenturl)
